...
import {useAnimatedValue, useLayout, useToggle} from '../hooks';
...
const PersonStagger: FC<PersonProps> = ({person, deletePressed}) => {
  ...
  const animValues = useAnimatedValue(balls.length)
  const startAnimations = useMemo(() => balls.map((notUsed, index) => Animated.spring(animValues[index], {useNativeDriver: true, toValue: 1})).reverse(), [])
  ...
  if (Platform.OS == 'ios')
    ...
    Animated.sequence([...startAnimations, ...endAnimations]).start(toggleStarted)}, [started])

  const icons = useMemo(() => balls.map((color, index) => {
    const numberOfIcons = balls.length
    const animValue = animValues[index]
    const animStyle = {
      transform: [{translateX: interpolate(animValue, [0, layout.width - numberOfIcons * iconSize])},
